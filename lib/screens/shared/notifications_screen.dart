...
/// Opening this screen marks everything currently unread as read.
...
class _NotificationsScreenState extends State<NotificationsScreen> {
  ...
  Future<List<AppNotification>> _load() async {
    ...
    final list = (rows as List).map((r) => AppNotification.fromMap(r as Map<String, dynamic>)).toList();

    final unreadIds = list.where((n) => n.readAt == null).map((n) => n.id).toList();
    if (unreadIds.isNotEmpty) {
      await supabase.from('notifications').update({'read_at': DateTime.now().toIso8601String()}).inFilter('id', unreadIds);
    }
    return list;
  }
