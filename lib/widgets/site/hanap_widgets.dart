...
class BadgePill extends StatelessWidget {
  ...
  const BadgePill({super.key, required this.text, this.showDot = true, this.dotColor = AppColors.gold});
    ...
    return Container(
      ...
      child: Row(
        ...
        children: [
          ...
          Text(text, style: AppText.body(size: 12, weight: FontWeight.w600, color: AppColors.gold)),
        ],
      ),
    );
  }
...
class FeatureCard extends StatefulWidget {
  ...
  const FeatureCard({super.key, required this.emoji, required this.title, required this.description, this.onTap});
    ...
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(color: _pressed ? AppColors.goldBorder : AppColors.hairline),
          boxShadow: _pressed
              ? [BoxShadow(color: AppColors.gold.withValues(alpha: 0.18), blurRadius: 30, offset: const Offset(0, 10))]
              : null,
        ),
        child: Column(
          ...
          children: [
            ...
            Text(widget.title, style: AppText.heading(size: 15, letterSpacing: -0.2)),
            const SizedBox(height: 6),
            Text(
              widget.description,
              style: AppText.body(size: 12.5, color: AppColors.textSecondary).copyWith(height: 1.5),
            ),
          ],
        ),
      ),
  ...
  Widget build(BuildContext context) {
    final rows = <List<StatEntry>>[
      for (var i = 0; i < stats.length; i += columns)
        stats.sublist(i, (i + columns > stats.length) ? stats.length : i + columns),
    ];
    return Container(
      decoration: BoxDecoration(
                ...
                for (var c = 0; c < rows[r].length; c++)
                  Expanded(
                    child: Container(
                      ...
                      decoration: BoxDecoration(
                        border: Border(
                          right: c != rows[r].length - 1 ? BorderSide(color: AppColors.hairline) : BorderSide.none,
                          bottom: r != rows.length - 1 ? BorderSide(color: AppColors.hairline) : BorderSide.none,
                        ),
                      ),
                      child: Column(
                        children: [
                          ...
                          AnimatedStatValue(value: rows[r][c].value, style: AppText.heading(size: 26, color: AppColors.gold)),
                          const SizedBox(height: 4),
                          Text(
                            rows[r][c].label.toUpperCase(),
                            textAlign: TextAlign.center,
                            style: AppText.body(size: 10.5, weight: FontWeight.w600, color: AppColors.textTertiary)
          ...
          text: TextSpan(
            ...
            children: [
              ...
              TextSpan(
                ...
                style: AppText.heading(size: 26, height: 1.2, color: AppColors.gold).copyWith(
                  shadows: [Shadow(color: AppColors.gold.withValues(alpha: 0.5), blurRadius: 20)],
                ),
              ),
            ],
          ),
          ...
          IntrinsicHeight(
            child: Row(
              ...
              children: [
                Column(
                  children: [
                    Container(
                      ...
                      child: Text("${i + 1}", style: AppText.heading(size: 14, color: AppColors.gold)),
                    ),
                    if (i != steps.length - 1)
                      Expanded(
                        child: Container(
                          ...
                          decoration: BoxDecoration(
                            gradient: LinearGradient(
                              ...
                              colors: [AppColors.goldBorder, AppColors.hairline],
                            ),
                          ),
                        ),
                      ),
                ...
                Expanded(
                  child: Padding(
                    padding: EdgeInsets.only(bottom: i == steps.length - 1 ? 0 : 28, top: 4),
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text(steps[i].title, style: AppText.heading(size: 16, letterSpacing: -0.2)),
                        const SizedBox(height: 5),
                        Text(
                          steps[i].description,
                          style: AppText.body(size: 13, color: AppColors.textSecondary).copyWith(height: 1.5),
                        ),
                      ],
                    ),
                  ),
...
class GlassNavBar extends StatelessWidget {
  ...
  const GlassNavBar({super.key, required this.scrolled, required this.menuOpen, required this.onMenuTap});
  ...
  Widget build(BuildContext context) {
    ...
    return ClipRect(
      child: BackdropFilter(
        filter: ImageFilter.blur(sigmaX: solid ? 16 : 0, sigmaY: solid ? 16 : 0),
        child: AnimatedContainer(
          duration: const Duration(milliseconds: 200),
          padding: EdgeInsets.symmetric(horizontal: 20, vertical: solid ? 14 : 18),
          decoration: BoxDecoration(
            color: solid ? AppColors.bg.withValues(alpha: 0.85) : Colors.transparent,
            border: Border(bottom: BorderSide(color: solid ? AppColors.hairline : Colors.transparent)),
          ),
          child: SafeArea(
            bottom: false,
            child: Row(
              ...
              children: [
                ...
                InkWell(
                  ...
                  child: Container(
                    ...
                    child: Icon(menuOpen ? Icons.close : Icons.menu, color: AppColors.gold, size: 20),
                  ),
                ),
              ],
            ),
